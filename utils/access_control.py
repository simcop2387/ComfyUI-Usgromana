...
GROUPS_CONFIG_FILE = os.path.join(os.path.dirname(__file__), "..", "users/usgromana_groups.json")

# Map Permission Keys -> URL Paths to Block (Prefixes)
# This blocks both the API calls AND the loading of the JS/CSS files
EXTENSION_BLOCK_MAP = {
...
class AccessControl:
    ...
    def _get_user_role_and_permissions(self, request):
        """Extracts user from request and resolves permissions."""
        # 1. Extract Token
        token = None
    ...
    # ---------------------------------------------------------
    # MAIN MIDDLEWARE (Replaces usgromana.py logic)
    # ---------------------------------------------------------
    ...
    def create_usgromana_middleware(self):
        ...
        async def middleware(request: web.Request, handler):
            ...
            # 1. PUBLIC WHITELIST (Always Allow)
            # Core functionality, login, static assets (except extensions), websocket
            if (
                path.startswith("/login")
                or path.startswith("/register")
                or path.startswith("/logout")
                or path.startswith("/usgromana")
            ...
            # 1b. Allow Core Extensions (Always)
            if (
                path.startswith("/extensions/core")
                or path.startswith("/extensions/ComfyUI-Usgromana")
            ):
                # Usgromana must always load so it can enforce UI restrictions,
                # even for guests.
                return await handler(request)
            ...
            is_api_ext = path.startswith("/api/extensions")  # The list of extensions

            # workflow API calls
            is_userdata_workflow = (
                path.startswith("/api/userdata/workflows")
                or path.startswith("/api/userdata/workflows:")
            )

            if is_queue and perms.get("can_run") is False:
                return web.json_response(
                    {"error": "Usgromana: Execution Denied"},
                    status=403,
                )

            if is_upload and perms.get("can_upload") is False:
                return web.json_response(
                    {"error": "Usgromana: Upload Denied"},
                    status=403,
                )

            # --- NEW: workflow save / modify protection (with popup-friendly response) ---
            if is_userdata_workflow and request.method in ("POST", "PUT", "DELETE", "PATCH"):
                # Read from perms; if missing, default: guests = blocked, others = allowed
                ...
                if not can_modify:
                    print(f"[Usgromana] BLOCKED {username} ({role}) from modifying {path}")
                    # This JSON is meant to be caught by frontend to show a popup.
                    return web.json_response(
                        {
                            "error": "Usgromana: Workflow Denied",
            ...
            # 4. EXTENSION & PATH BLOCKING (The "View" Logic)
            # We iterate our map. If the permission key exists in config and is FALSE,
            # we check if the current path matches the blocked paths.
            ...
            for perm_key, blocked_paths in EXTENSION_BLOCK_MAP.items():
                ...
                # Default Logic:
                # If undefined, Guest = Block (false), Others = Allow (true)
                if allow is None:
                    allow = (role != "guest")
                ...
                if allow is False:
                    for blocked_prefix in blocked_paths:
                        if path.lower().startswith(blocked_prefix.lower()):
                            print(f"[Usgromana] BLOCKED {username} ({role}) from {path}")
                            return web.Response(
                                status=403,
                                text="Usgromana: Access Denied",
                            )

            # 5. API/EXTENSIONS LIST BLOCKING
            if not is_queue and not is_upload and path.startswith("/api/"):
                if perms.get("can_access_api") is False:
                    return web.json_response(
                        {"error": "Usgromana: API Denied"},
                        status=403,
                    )
    ...
    # ---------------------------------------------------------
    # USER CONTEXT & FOLDER LOGIC (Existing)
    # ---------------------------------------------------------
    ...
    def create_folder_access_control_middleware(self):
        ...
        async def middleware(request: web.Request, handler):
            if not request.path.startswith(folder_paths_check):
                return await handler(request)
            user_id = request.get("user_id")  # This usually comes from query params in file requests
            # Simple check
            return await handler(request)
    ...
    def user_queue_put(self, item):
            ...
            if os.path.exists(GROUPS_CONFIG_FILE):
                try:
                    ...
                    if perms.get("can_run") is False:
                        print(f"[AccessControl] Blocked execution for {current_user_id} (Role: {role})")
                        return
                except Exception as e:
                    print(f"Error checking permissions in queue: {e}")
        ...
        # NOTE: keep underlying behavior as in your real implementation
    ...
    def create_manager_access_control_middleware(self, manager_directory="/extensions/comfyui-manager", manager_routes=()):
        ...
        async def middleware(request: web.Request, handler):
            ...
            admin_id = self.users_db.get_admin_user()[0]
